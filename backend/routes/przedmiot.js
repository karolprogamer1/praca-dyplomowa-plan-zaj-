...
router.put('/przedmiot/:id', async (req,res) =>{
    try{
        ...
        const result = await pool.query(
          'UPDATE przedmiot SET wykladowca_id = $1,nazwa = $2, typ = $3 , ilosc_godz = $4 WHERE idprzedmiotu = $5'
          [
              wykladowca_id || Null,
              nazwa || Null,
              typ || Null,
...
router.delete('/przedmiot/:id',async (req,res) =>{
    try{
        ...
        const result = await pool.query('DELETE FROM przedmiot * WHERE idprzedmiotu = $1',[idNum]);
