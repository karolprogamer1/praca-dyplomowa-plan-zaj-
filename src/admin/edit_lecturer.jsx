...
export default function EditLecturer({ style, title = 'Edytuj wykładowcę', lecturer, formData = {}, status = {}, onChange, onCancel, onSave }) {
  ...
  const handleSubmit = (e) => {
    ...
    onSave({
      ...
      // dane do konta (jeśli frontend wysyła)
      ...
      <div className="student-plan-card admin-form-card">
        <div className="plan-header">
          <div>
            ...
            <h1>{title}</h1>
            <p className="plan-subtitle">Zmień dane wykładowcy i zapisz.</p>
          </div>
        </div>
        {status?.message && (
        ...
        <form className="admin-student-form" onSubmit={handleSubmit}>
          <label>
            Użytkownik ID
            <input type="text" inputMode="numeric" pattern="[0-9]*" name="userId" placeholder="ID użytkownika" value={formData.userId || ''} onChange={handleChange} />
          </label>
          <label>
            Imię
